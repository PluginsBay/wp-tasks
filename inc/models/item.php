...
class task_item extends tzn_model
{
	...
	public function check() {
		...
		if (empty($this->data['title'])) {
			$msg['title'] = __('Title should not be blank', 'tasks');
		}
		if (!preg_match('/^[123]$/', $this->data['priority'])) {
			$msg['priority'] = __('Priority should be 1, 2, or 3', 'tasks');
		}
		if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $this->data['deadline_date'])) {
			$msg['deadline_date'] = __('Invalid date format', 'tasks');
		}
		if (strlen($this->data['description']) > 65535) {
			$msg['description'] = __('Description is too big (maybe you pasted an image into it?)', 'tasks');
		}
		
		foreach ($msg as $k => $v) {
			$this->errors[$k] = '<p class="task_err">'.__($v, 'tasks').'</p>';
		}
		return empty($msg);
	}
	 ...
	 * @return task_item_status
	 */
	public function get_status() {
		$obj = new task_item_status();
		if ($pid = $this->get_uid()) {
	...
	public function get_deadline_proximity() {
		if ($this->data['deadline_date'] == '0000-00-00 00:00:00')
			return __('Undefined Deadline', 'tasks');
		...
		if ($date_diff['sec'] < 0) {
			return __("Past due !", 'tasks');
		}
		if ($date_diff['sec'] < 3600) {
			return sprintf(_n("%s minute remaining", "%s minutes remaining", $date_diff['min'], 'tasks'), $date_diff['min']);
		}
		if ($date_diff['sec'] < 86400) {
			return sprintf(_n("%s hour remaining", "%s hours remaining", $date_diff['hour'], 'tasks'), $date_diff['hour']);
		}
		if ($date_diff['sec'] < 365 * 86400) {
			return sprintf(_n("%s day remaining", "%s days remaining", $date_diff['day'], 'tasks'), $date_diff['day']);
		}
		return sprintf(_n("%s month remaining", "%s months remaining", $date_diff['month'], 'tasks'), $date_diff['month']);
	}
...
class task_item_status extends task_log {
	...
	public static function get_status_list($draft, $context='many tasks') {
		$arr = array(
    		0 		=> _x('Draft', $context, 'tasks'),
    		20 		=> _x('In Progress', $context, 'tasks'),
    		30 		=> _x('Suspended', $context, 'tasks'),
    		60 		=> _x('Closed', $context, 'tasks')
		);
    	if (!$draft) {
	    	unset($arr[0]);
    	}
	...
	public function get_status() {
		...
		switch($key) {
			case 0:
				return __('Draft', 'tasks');
			case 20:
				return __('In Progress', 'tasks');
			case 30:
				return __('Suspended', 'tasks');
			case 60:
				return __('Closed', 'tasks');
			default:
				return __('Unknown Status !', 'tasks');
		}
	}
    ...
    public static function list_links($url, $name, $current) {
	    return  tzn_tools::form_links($url, $name, array('all' => _x('All tasks', 'tasks', 'tasks')), $current) 
	    		.tzn_tools::form_links($url, $name, self::get_status_list(is_user_logged_in()), $current);
    }
	
}

class task_item_comment extends tzn_model
{
	public function __construct() {
		parent::__construct(
				'item_comment',
	...
	public function check() {
		if (empty($this->data['body'])) {
			$this->errors["body"] = '<p class="task_err">'.__('Empty comment not allowed.', 'tasks').'</p>';
			return false;
		}
		return true;
	}
...
class task_item_file extends tzn_model
{
	public function __construct() {
		$this->error = '';
		parent::__construct(
	...
	public function upload($file_input) {
		...
		if ($_FILES[$file_input]['error'] == UPLOAD_ERR_OK) {
			...
			|| preg_match('/\.(php|phtml|phps)$/i', $_FILES[$file_input]['name'])) {
				$this->error = __('Forbidden upload: ', 'tasks').$_FILES[$file_input]['name'];
				return false;
			} else {
				...
				if (!@move_uploaded_file($_FILES[$file_input]['tmp_name'], $wp_upload_dir['path'].'/'.$file_name)) {
					$this->error = __('Unable to write file to upload directory. Please tell an admin.', 'tasks');
					return false;
				} else {
			...
			$this->error = __('Upload of ', 'tasks').$_FILES[$file_input]['name'].__(' failed with error code ', 'tasks').$_FILES[$file_input]['error'];
			return false;
		}
	}
}

class task_item_info extends task_item
{
	public function __construct() {
		parent::__construct();
	...
	public function get_attachments() {
		return sprintf(_n('%d attachment', '%d attachments', $this->data['file_count'], 'tasks'), $this->data['file_count']);
	}
}

class task_item_status_info extends task_item_status {
	...
	public function load_list($args=null) {
		$sql = array(
				...
				'having'=> task_user::get_roles_sql('who_read').' AND status > 0',
				'order' => 'log_date DESC'
		);
