...
class task_project extends tzn_model
{
	...
	public function check_access($what='read', $user_id=null) {
		...
		if ($r = $this->get($what)) {
			if (task_user::check_role($r, $user)) {
				// all is good
				return true;
			} else {
...
class task_project_status extends task_log
{
	...
	public static function get_status_list($draft) {
		$arr = array(
    		0 		=> _x('Draft', 'many projects', 'tasks'),
    		20 		=> _x('In Progress', 'many projects', 'tasks'),
    		30 		=> _x('Suspended', 'many projects', 'tasks'),
    		60 		=> _x('Closed', 'many projects', 'tasks')
    	);
    	if (!$draft) {
	    	unset($arr[0]);
    	}
    ...
    public static function list_links($url, $name, $current) {
	    return 	 tzn_tools::form_links($url, $name, array('all' => _x('All', 'projects', 'tasks')), $current)
	    		.tzn_tools::form_links($url, $name, self::get_status_list(current_user_can('manage_options')), $current);
    }
	
}

class task_project_user extends tzn_model 
{
	...
	public function get_position($pos=null) {
		...
		switch ($pos) {
			case 0:
				return __('Guest', 'tasks');
			case 1:
				return __('Client', 'tasks');
			case 2:
				return __('Staff', 'tasks');
			case 3:
				return __('Manager', 'tasks');
			case 4:
				return __('Project Leader', 'tasks');
		}
	}
...
class task_project_info extends task_project
{
