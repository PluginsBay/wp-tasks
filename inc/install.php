...
register_activation_hook(TASK_ROOT_FILE, 'task_install');
...
$arr = array(
	...
	'task_all'		=> 'projects', // projects, tasks, recent, dashboard (PRO)
	'tasks_per_page'	=> 5,
	'format_date'		=> 'Y-m-d',
	'format_time'		=> 'H:i',
	'number_updates'	=> 7,
...
add_option('task_options', $arr, '', 'no'); // no = not autoloaded

// create custom tables (if needed)

require_once(ABSPATH.'wp-admin/includes/upgrade.php');

$table = $wpdb->prefix.'task_item';
...
$table = $wpdb->prefix.'task_item_status';
$sql = "CREATE TABLE $table (
...
$table = $wpdb->prefix.'task_item_like';
$sql = "CREATE TABLE $table (
...
$table = $wpdb->prefix.'task_item_comment';
$sql = "CREATE TABLE $table (
...
$table = $wpdb->prefix.'task_item_comment_like';
$sql = "CREATE TABLE $table (
...
$table = $wpdb->prefix.'task_item_file';
$sql = "CREATE TABLE $table (
...
$table = $wpdb->prefix.'task_project';
$sql = "CREATE TABLE $table (
...
$table = $wpdb->prefix.'task_project_user';
$sql = "CREATE TABLE $table (
  project_id mediumint(8) unsigned NOT NULL DEFAULT '0',
  user_id mediumint(8) unsigned NOT NULL DEFAULT '0',
...
$table = $wpdb->prefix.'task_project_status';
$sql = "CREATE TABLE $table (
...
register_deactivation_hook(TASK_ROOT_FILE, 'task_deactivate');

/**
 * called when deactivating (not deleting) plugin
 */
function task_deactivate() {
	// remove options
