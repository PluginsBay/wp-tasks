...
if (isset($_REQUEST['npg']) && preg_match('/^\d+$/', $_REQUEST['npg'])) {
	if (!headers_sent()) {
		setcookie('task_page_size', $_REQUEST['npg']);
	}
	$this->page_size = $_REQUEST['npg'];
} elseif (isset($_COOKIE['task_page_size']) && preg_match('/^\d+$/', $_COOKIE['task_page_size'])) {
	$this->page_size = $_COOKIE['task_page_size'];
...
// load project
$this->project = new task_project();
$this->project->set_uid($this->pid);
if (!$this->pid || !$this->project->load()) {
	echo '<p class="task_err">'.__("No such project.", 'tasks').'</p>';
	return;
} else {
	// check if user has access
	if (!$this->project->check_access('read')) {
		echo '<p class="task_err">'.__("Sorry, you can't access this project. Please contact an admin.", 'tasks').'</p>';
		return;
	}
}
