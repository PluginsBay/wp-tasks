...
if (!empty($_REQUEST['action'])) {
	...
	// check nonce
	check_admin_referer('task_project_'.$_REQUEST['action']);

	if ($_REQUEST['action'] == 'trash') {
		// move to trash
		$this->data->set('trashed',1);
		$this->data->update();
		wp_redirect(admin_url().'admin.php?page=tasks_projects');
		exit;
	}
	if ($_REQUEST['action'] == 'restore') {
		...
		$this->data->update();
		wp_redirect(admin_url().'admin.php?page=tasks_projects&filter=trash');
		exit;
	}
...
// load status history
$this->status = new task_project_status();
if ($this->pid) {
...
wp_register_script('tznadminjs', plugins_url( '/js/admin.js' , TASK_ROOT_FILE), array('jquery') );
