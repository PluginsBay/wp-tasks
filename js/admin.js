...
function task_project_user_add(id) {
	// add new row in user's table
	event.preventDefault();
	var uid = jQuery('#task_user_id').val();
	var unm = jQuery('#task_user_id :selected').text();
	var pos = jQuery('#task_position').val();
	var pnm = jQuery('#task_position :selected').text();
	// alert('adding '+uid+' = '+unm+' as '+pnm+' ('+pid+') on project '+id);
	jQuery.get(ajaxurl+'?action=task_project_user_add', {'id': id, 'uid': uid, 'pos': pos, 'unm': unm, 'pnm': pnm}, function(data) {
		jQuery('#task_project_users tbody').append(data);
	});
}

function task_project_user_edit(id, uid) {
	// update data in corresponding row
	event.preventDefault();
	var unm = jQuery('#task_user_name').text();
	var pos = jQuery('#task_position').val();
	var pnm = jQuery('#task_position :selected').text();
	jQuery.get(ajaxurl+'?action=task_project_user_edit', {'id': id, 'uid': uid, 'pos': pos, 'unm': unm, 'pnm': pnm}, function(data) {
		jQuery('#task_project_users tbody #pos-'+uid).empty().append(data);
	});
}

function task_project_user_delete(uid,tmp) {
	// remove line from table of users
	jQuery('#pos-'+uid).remove();
	// add uid in list of users to delete
	if (tmp) {
		var elm = jQuery('#task_project_users_delete');
		var elv = elm.val();
		if (elv) {
			elv += ',';
		}
